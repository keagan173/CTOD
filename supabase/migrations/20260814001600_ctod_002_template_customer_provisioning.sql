-- CTOD 002 owner-only template-selected customer provisioning
create or replace function public.operator_service_provision_customer_v2(
  p_actor_user_id uuid,p_name text,p_slug text,p_timezone text,p_owner_email text,p_plan_code text,
  p_provisioning_key text,p_trial_days integer,p_template_code text,p_template_version_code text,p_request_id uuid
)
returns jsonb language plpgsql security definer set search_path='' as $$
declare
  v_company_id uuid; v_prior_company_id uuid; v_invite_id uuid; v_token uuid;
  v_email text:=lower(nullif(btrim(p_owner_email),'')); v_core_version text; v_template_code text; v_template_version text;
begin
  ...
  insert into private.customer_accounts(company_id,account_status,plan_code,trial_ends_at,core_version,deployment_status,backup_status,health_status,updated_at)
  values(v_company_id,'trial',p_plan_code,case when p_trial_days>0 then now()+make_interval(days=>p_trial_days) else null end,coalesce(v_core_version,'1.0.1'),'provisioning','not_verified','setup_required',now())
  on conflict(company_id) do update set plan_code=excluded.plan_code,trial_ends_at=coalesce(private.customer_accounts.trial_ends_at,excluded.trial_ends_at),core_version=coalesce(private.customer_accounts.core_version,excluded.core_version),updated_at=now();
  if v_email is not null then
    ...
    perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(v_company_id::text||'|'||v_email||'|owner',0));
    ...
    if v_invite_id is null then
      insert into public.access_invites(company_id,email,intended_role,invited_by_user_id,expires_at)
      values(v_company_id,v_email,'owner',p_actor_user_id,now()+interval '14 days') returning id,token into v_invite_id,v_token;
    else
      update public.access_invites set expires_at=now()+interval '14 days',invited_by_user_id=p_actor_user_id where id=v_invite_id;
    end if;
  end if;
  ...
  values(p_request_id,p_actor_user_id,v_company_id,case when v_prior_company_id is null then 'customer.provisioned_from_template' else 'customer.provision_retried' end,'company',v_company_id,jsonb_build_object('name',btrim(p_name),'slug',p_slug,'timezone',p_timezone,'plan_code',p_plan_code,'owner_email',v_email,'template_code',v_template_code,'template_version',v_template_version)) on conflict(request_id) do nothing;
