-- CTOD 002 staged platform rollout services
...
begin
  ...
  with chosen as (
    select ca.company_id from private.customer_accounts ca where ca.account_status not in ('closed') and (p_rollout_stage='all' or ca.company_id=any(coalesce(p_company_ids,'{}'::uuid[])))
  ), upserted as (
    ...
    on conflict(release_id,company_id) do update set rollout_stage=excluded.rollout_stage,status='scheduled',reason=excluded.reason,scheduled_by_user_id=excluded.scheduled_by_user_id,scheduled_at=now(),activated_at=null,rolled_back_at=null,updated_at=now()
  ...
  if v_count=0 then raise exception 'No eligible customers selected for rollout'; end if;
...
begin
  ...
  if p_request_id is null then raise exception 'Request ID is required'; end if;
  select version_code into v_version from private.platform_releases where id=p_release_id and status='available';
  if not found then raise exception 'Platform release is not available'; end if;
  with chosen as (
    select t.company_id from private.platform_release_targets t where t.release_id=p_release_id and t.status='scheduled' and (coalesce(array_length(p_company_ids,1),0)=0 or t.company_id=any(p_company_ids))
  ), changed as (
  ...
  get diagnostics v_count=row_count;
  if v_count=0 then raise exception 'No scheduled customers were eligible for activation'; end if;
  insert into private.customer_release_history(company_id,actor_user_id,action,from_version,to_version,reason)
...
begin
  if not exists(select 1 from private.platform_operators where user_id=p_actor_user_id and active=true and operator_role='platform_admin') then raise exception 'Platform administrator access required'; end if;
  if p_request_id is null then raise exception 'Request ID is required'; end if;
  select version_code into v_version from private.platform_releases where id=p_release_id;
  if not found then raise exception 'Platform release not found'; end if;
  with chosen as (
    select t.company_id from private.platform_release_targets t join private.customer_accounts ca on ca.company_id=t.company_id where t.release_id=p_release_id and t.status='active' and ca.core_version=v_version and ca.previous_core_version is not null and (coalesce(array_length(p_company_ids,1),0)=0 or t.company_id=any(p_company_ids))
  ), changed as (
  ...
  get diagnostics v_count=row_count;
  if v_count=0 then raise exception 'No active customers were eligible for rollback'; end if;
  insert into private.customer_release_history(company_id,actor_user_id,action,from_version,to_version,reason)
