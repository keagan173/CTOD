-- CTOD 002 only. Ensure newly provisioned tenants always receive a usable rating scale.
-- Published templates may provide configuration.ratings. If omitted, CTOD seeds the standard 5-level scale.
...
create or replace function private.provision_company_from_template(
  ...
  p_template_version_code text default null::text
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  select r.company_id,r.status into v_company_id,v_existing_status
  from public.company_provisioning_runs r where r.provisioning_key=p_provisioning_key;
  if v_existing_status='completed' and v_company_id is not null then return v_company_id; end if;

  select t.id,v.id,v.version_code,v.schema_version,v.configuration
  into v_template_id,v_template_version_id,v_template_version,v_schema_version,v_configuration
  ...
  order by case when p_template_version_code is not null and v.version_code=p_template_version_code then 0 else 1 end,
  ...
  values(upper(btrim(p_template_code)),btrim(p_name),p_slug,p_timezone,'active','{}'::jsonb,v_template_id,v_template_version_id,now())
  ...
  values(v_company_id,v_schema_version,upper(btrim(p_template_code))||' Template '||v_template_version,'published'::public.config_status,v_schema_version,now())
  ...
  for v_question in select value from jsonb_array_elements(coalesce(v_configuration->'questions','[]'::jsonb)) loop
    v_role_id:=null;
    if nullif(btrim(v_question->>'role_title'),'') is not null then
      select id into v_role_id from public.roles where company_id=v_company_id and title=btrim(v_question->>'role_title') limit 1;
    end if;
  ...
  if jsonb_typeof(v_configuration->'ratings')='array' and jsonb_array_length(v_configuration->'ratings')>0 then
    ...
    insert into public.rating_scale_items(company_id,config_version_id,code,label,score_value,sort_order,employee_visible)
    values
      (v_company_id,v_config_version_id,'5','Exceptional',5,10,true),
      (v_company_id,v_config_version_id,'4','Exceeds Expectations',4,20,true),
      (v_company_id,v_config_version_id,'3','Meets Expectations',3,30,true),
      (v_company_id,v_config_version_id,'2','Needs Improvement',2,40,true),
      (v_company_id,v_config_version_id,'1','Unsatisfactory',1,50,true)
