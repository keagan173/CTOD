alter table public.career_decisions add column if not exists specialist_growth_path text;

-- Work preference is intentionally binary for leadership analytics.
...
returns public.career_decisions language plpgsql security definer set search_path='public','private','pg_catalog' as $function$
declare v public.reviews; c public.career_decisions; begin select * into v from public.reviews where id=p_review_id; if not found then raise exception 'Review not found'; end if; if not private.can_access_location(v.company_id,v.location_id) then raise exception 'Access denied'; end if; if v.status='finalized' then raise exception 'Finalized review cannot be changed'; end if; if p_promotion_readiness not in ('Ready Now','30-90 Days','Within 1 Year','Not Yet Ready') then raise exception 'Invalid promotion readiness'; end if; insert into public.career_decisions(company_id,review_id,employee_id,promotion_readiness) values(v.company_id,v.id,v.employee_id,p_promotion_readiness) on conflict(review_id) do update set promotion_readiness=excluded.promotion_readiness returning * into c; return c; end $function$;
revoke all on function public.save_review_promotion_readiness(uuid,text) from public; grant execute on function public.save_review_promotion_readiness(uuid,text) to authenticated;

create or replace function public.save_review_specialist_growth(p_review_id uuid,p_specialist_growth_path text)
returns public.career_decisions language plpgsql security definer set search_path='public','private','pg_catalog' as $function$
declare v public.reviews; c public.career_decisions; begin select * into v from public.reviews where id=p_review_id; if not found then raise exception 'Review not found'; end if; if not private.can_access_location(v.company_id,v.location_id) then raise exception 'Access denied'; end if; if v.status='finalized' then raise exception 'Finalized review cannot be changed'; end if; if p_specialist_growth_path not in ('Training / certifications','Mentorship / training others','More responsibility in current specialty','Management responsibility','Higher-level specialist / broader organizational scope','Advanced technical mastery','Cross-functional specialist experience') then raise exception 'Invalid specialist growth path'; end if; insert into public.career_decisions(company_id,review_id,employee_id,specialist_growth_path) values(v.company_id,v.id,v.employee_id,p_specialist_growth_path) on conflict(review_id) do update set specialist_growth_path=excluded.specialist_growth_path returning * into c; return c; end $function$;
revoke all on function public.save_review_specialist_growth(uuid,text) from public; grant execute on function public.save_review_specialist_growth(uuid,text) to authenticated;