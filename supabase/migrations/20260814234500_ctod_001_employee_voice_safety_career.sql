-- 001 second-half employee voice questions, positioned before compensation discussion.
alter table public.career_decisions
 add column if not exists safety_priority_response text,
 add column if not exists career_feeling_response text;
...
begin
 ...
 on conflict(review_id) do update set safety_priority_response=excluded.safety_priority_response,career_feeling_response=excluded.career_feeling_response
 returning * into c;
 return c;
end $function$;
revoke all on function public.save_review_employee_voice(uuid,text,text) from public;
grant execute on function public.save_review_employee_voice(uuid,text,text) to authenticated;