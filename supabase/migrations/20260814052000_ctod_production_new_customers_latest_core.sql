...
begin
 ...
 values(v_company_id,'trial',p_plan_code,case when p_trial_days>0 then now()+make_interval(days=>p_trial_days) else null end,coalesce(v_core_version,'1.0.1'),'current',now())
 on conflict(company_id) do update set plan_code=excluded.plan_code,trial_ends_at=coalesce(private.customer_accounts.trial_ends_at,excluded.trial_ends_at),core_version=case when v_prior_company_id is null then excluded.core_version else private.customer_accounts.core_version end,updated_at=now();
 ...
 values(p_request_id,p_actor_user_id,v_company_id,case when v_prior_company_id is null then 'customer.provisioned_from_template' else 'customer.provision_retried' end,'company',v_company_id,jsonb_build_object('name',btrim(p_name),'slug',p_slug,'plan_code',p_plan_code,'owner_email',v_email,'template_code',v_template_code,'template_version',v_template_version,'core_version',(select core_version from private.customer_accounts where company_id=v_company_id))) on conflict(request_id) do nothing;
 return jsonb_build_object('company_id',v_company_id,'account_status',(select account_status from private.customer_accounts where company_id=v_company_id),'core_version',(select core_version from private.customer_accounts where company_id=v_company_id),'template_code',v_template_code,'template_version',v_template_version,'invite_id',v_invite_id,'invite_token',v_token,'owner_email',v_email,'reused',v_prior_company_id is not null);
end $$;
