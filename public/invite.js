...
async function validateInvite(){
  ...
  try{
    ...
    if(!r.ok||!data.ok){$('#inviteMsg').textContent=data.error||'Invite is invalid.';return}
    if(!data.active){$('#inviteMsg').textContent=data.accepted?'This invitation has already been activated.':data.expired?'This invitation has expired.':data.revoked?'This invitation was revoked.':'Invite is no longer active.';return}
  ...
  }catch(e){$('#inviteMsg').textContent='Could not verify invitation. Check your connection and try again.'}
}
...
async function activateInvite(){
  ...
  $('#activateInvite').disabled=true;$('#inviteMsg').textContent='Activating CTOD access...';
  try{
    ...
    if(s.error){$('#inviteMsg').textContent='Access activated. Opening CTOD sign in...';setTimeout(()=>location.href='/',900);return}
    $('#inviteMsg').textContent='Access activated. Opening your CTOD dashboard...';setTimeout(()=>location.href='/',650);
  }catch(e){$('#inviteMsg').textContent='Activation failed. Please try again.';$('#activateInvite').disabled=false}
}
