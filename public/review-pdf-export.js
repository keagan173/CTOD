const PDF_VERSION='20260815-pdf2';
...
function exportFrame(source,logoData){
  ...
  Object.assign(wrap.style,{
    ...
    margin:'0',padding:'0',background:'#fff',overflow:'hidden',zIndex:'-1',
  ...
  Object.assign(page.style,{
    ...
    background:'#fff',pageBreakAfter:'auto'
  });
  page.querySelectorAll('*').forEach(el=>{el.style.transform='none';el.style.zoom='1';});
  const head=page.querySelector('.brandhead');
  if(head){
    Object.assign(head.style,{height:'70px',minHeight:'70px',maxHeight:'70px',padding:'10px 12px',overflow:'hidden'});
  }
  const logo=page.querySelector('.brandhead img');
  if(logo){
    if(logoData) logo.src=logoData;
    Object.assign(logo.style,{width:'145px',height:'48px',maxWidth:'145px',maxHeight:'48px',objectFit:'contain',objectPosition:'left center',display:'block'});
