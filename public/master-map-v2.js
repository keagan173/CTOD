...
async function ensureLeaflet(){
 if(window.L)return;
 if(!document.querySelector('link[data-ctod-leaflet]')){
  const css=document.createElement('link');css.rel='stylesheet';css.href='/vendor/leaflet/leaflet.css?v=1.9.4';css.dataset.ctodLeaflet='1';document.head.appendChild(css);
 }
 await new Promise((resolve,reject)=>{
  let script=document.querySelector('script[data-ctod-leaflet]');
  const timeout=setTimeout(()=>reject(new Error('Local map library timed out')),8000);
  const loaded=()=>{clearTimeout(timeout);window.L?resolve():reject(new Error('Local map library did not initialize'))};
  const failed=()=>{clearTimeout(timeout);reject(new Error('Local map library could not load'))};
  if(script){script.addEventListener('load',loaded,{once:true});script.addEventListener('error',failed,{once:true});return}
  script=document.createElement('script');script.src='/vendor/leaflet/leaflet.js?v=1.9.4';script.dataset.ctodLeaflet='1';script.onload=loaded;script.onerror=failed;document.head.appendChild(script);
 });
}
...
]);for(const result of [l,a,r,c,p])if(result.error)throw result.error;return{locations:l.data||[],assignments:a.data||[],reviews:r.data||[],coach:c.data||[],promo:p.data||[]}}
...
let timer=0,rendering=false,mapInstance=null,expectedCount=0;
async function upgrade(force=false){const old=document.querySelector('#ctodMap');if(!old||rendering)return;const markerCount=old.querySelectorAll('.leaflet-marker-icon').length;const healthyV2=old.dataset.mapV2==='1'&&expectedCount>0&&markerCount===expectedCount;if(!force&&healthyV2){mapInstance?.invalidateSize();return}rendering=true;try{styles();const d=await data();const mapped=d.locations.filter(coords);expectedCount=mapped.length;await ensureLeaflet();if(mapInstance){try{mapInstance.remove()}catch{}mapInstance=null}const fresh=document.createElement('div');fresh.id='ctodMap';fresh.dataset.mapV2='1';fresh.style.height='430px';fresh.style.borderRadius='14px';fresh.style.overflow='hidden';old.replaceWith(fresh);
 ...
 const map=L.map(fresh,{zoomControl:true,attributionControl:true});mapInstance=map;L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap &copy; CARTO'}).addTo(map);const group=L.layerGroup().addTo(map);
 ...
 }catch(e){console.error('CTOD Master Map v2',e);const host=document.querySelector('#ctodMap');if(host)host.innerHTML='<div style="display:grid;place-items:center;height:100%;padding:24px;text-align:center;color:#9bb5c9">The location map could not load. The location directory and performance matrix remain available below.</div>'}finally{rendering=false}}
function schedule(){clearTimeout(timer);timer=setTimeout(()=>upgrade(false),220)}
window.ctodRenderMasterMap=upgrade;
document.addEventListener('ctod:master-rendered',schedule);
document.querySelector('#tabMaster')?.addEventListener('click',schedule);
schedule();
