...
'lagrande|OR':[45.3246,-118.0877],'ontario|OR':[44.0266,-116.9629],'baker city|OR':[44.7749,-117.8344],
...
const cleanCity=s=>String(s||'').trim().toLowerCase().replace(/\./g,'').replace('mountain home','mtn home');
...
let timer=0,rendering=false,lastHost=null;
async function upgrade(){const old=document.querySelector('#ctodMap');if(!old||rendering||old===lastHost&&old.dataset.mapV2==='1')return;rendering=true;try{styles();const d=await data();await ensureLeaflet();const fresh=document.createElement('div');fresh.id='ctodMap';fresh.dataset.mapV2='1';fresh.style.height='430px';fresh.style.borderRadius='14px';fresh.style.overflow='hidden';old.replaceWith(fresh);lastHost=fresh;
...
function schedule(){clearTimeout(timer);timer=setTimeout(upgrade,220)}
